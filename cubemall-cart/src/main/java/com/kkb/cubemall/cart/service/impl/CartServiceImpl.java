...
public class CartServiceImpl implements CartService {
    ...
    public CartItemVo addToCart(Long skuId, Integer num) {
        ...
        // 1、获取购物车数据，不论用户是否登录 在redis中标识了不同的cartkey
    ...
    public BoundHashOperations<String,Long,Object> getCartFromRedis() {
        ...
        // 1、获取购物车中已经存在的数据 （需要判断该用户是否是临时用户）
        UserInfoDTO userInfoDTO = CartInterceptor.dtoThreadLocal.get();
        String cartKey = "";
        // 判断是否登录 根据用户是否登录决定放入redis中的key是否是临时key
        if(userInfoDTO.getUserId()!=null){
        ...
        // 从redis中获取购物车相关的数据
        /**
         * String键值 存储cartKey   <long,Object>是一个map结构 skuId cartItem
         */
